...
import { TitleInput } from "./SignupForm";
...
const EditResume = () => {
  ...
  //     it will help in choosing the preview as well as helps in downloading
  // the resume and also saves resume as a image
  ...
  const uploadResumeImages = async () => {
    try {
      ...
      const uploadResponse = await axiosInstances.put(
  ...
  const updateResumeDetails = async (thumbnailLink) => {
    try {
      ...
      await axiosInstances.put(API_PATHS.RESUME.UPDATE(resumeId), {
        ...resumeData,
  ...
  const handleDeleteResume = async () => {
    try {
      setIsLoading(true);
      await axiosInstances.delete(API_PATHS.RESUME.DELETE(resumeId));
  ...
  useEffect(() => {
    if (resumeId) {
      ...
      console.error(
        "🚫 resumeId is missing from URL. Redirecting to dashboard."
      );
      toast.error("Invalid link: Resume ID is missing.");
    }
  }, [resumeId]);
  //    so all this function used for the edit resumes
  return (
    <DashboardLayout>
